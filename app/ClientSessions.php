...
class ClientSessions extends Model
{
    ...
    public static function getPunchCount()
    {
        ...
        $punchesCount = self::select(\DB::raw('SUM(punches_count) as punch_count'))->where('client_id', \Auth::user()->id)
    ...
    {
        $userParticpation = self::where('client_id', $userId)
    ...
    {
        $speedDemon = self::where('client_id', $userId)
                ->where(function($query) {
                    $query->whereNull('battle_id')->orWhere('battle_id', '0');
                })
    ...
    {
        $returnData = self::where('client_id', $userId)
                ->where(function($query) {
                    $query->whereNull('battle_id')->orWhere('battle_id', '0');
                })
    ...
    private static function doSessionComparison($comboPunches, $session)
    {
        ...
        foreach ($sessions as $session) {
            ...
            foreach ($rounds as $round) {
                $roundPunches[$session->client_id] = [];
